...
func (w *Worker) handleGyroscopeMsg(msg *nats.Msg) {
	...
	if err := json.Unmarshal(msg.Data, &data); err != nil {
		log.Printf("ERRO: Falha ao decodificar mensagem de giroscópio: %v", err)
		msg.Term()
	...
	log.Printf("Mensagem de giroscópio para device %s processada.", data.DeviceID)
	msg.Ack()
...
func (w *Worker) handleGpsMsg(msg *nats.Msg) {
	...
	if err := json.Unmarshal(msg.Data, &data); err != nil {
		log.Printf("ERRO: Falha ao decodificar mensagem de GPS: %v", err)
		msg.Term()
		metrics.NatsMessagesProcessed.WithLabelValues(subject, "terminated").Inc()
		return
	}
	...
	log.Printf("Mensagem de GPS para device %s processada.", data.DeviceID)
	msg.Ack()
	metrics.NatsMessagesProcessed.WithLabelValues(subject, "success").Inc()
}
func (w *Worker) handlePhotoMsg(msg *nats.Msg) {
	...
	if err := json.Unmarshal(msg.Data, &data); err != nil {
		log.Printf("ERRO: Falha ao decodificar mensagem de foto: %v", err)
		msg.Term()
		metrics.NatsMessagesProcessed.WithLabelValues(subject, "terminated").Inc()
		return
	}
...
func main() {
	...
	if err := db.InitTables(); err != nil {
		log.Fatalf("ERRO: Não foi possível inicializar as tabelas a partir do worker: %v", err)
	}
